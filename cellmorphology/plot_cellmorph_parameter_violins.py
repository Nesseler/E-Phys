...
for col in cellmorph_parameters.columns:
    if 'axon' in col:
        cellmorph_parameters[col].replace(to_replace = 0,
                                          value = np.nan,
                                          inplace = True)
...
for col in cellmorph_parameters_MeA:
    ...
    if (MeA_normal_pvalue > 0.05 and BAOT_normal_pvalue > 0.05):
        ...
        if mannwhitneyu_res.pvalue < 0.05:
            cellmorph_parameters_between_regions.at[f'{col}', 'statistical_difference'] = True
        else:
            cellmorph_parameters_between_regions.at[f'{col}', 'statistical_difference'] = False

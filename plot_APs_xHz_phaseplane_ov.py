# -*- coding: utf-8 -*-
"""
Created on Thu Jan 11 18:18:23 2024
...
from spiketrains_functions import get_colorcode, get_AP_parameters
...
cell_path = os.path.join(directories.quant_data_dir, 'APs', cell_ID, f'{cell_ID}_{frequency}.xlsx')

AP_params = pd.read_excel(cell_path, index_col='idx_step')
...
stim_start_idx = int((PGF_parameters['t_pre'] - 5) * SR_ms)
stim_stop_idx = int((PGF_parameters['t_pre'] + PGF_parameters['t_stim'] + 5) * SR_ms) + 1
...
norm = mtl.colors.CenteredNorm(0, 200)
...
cc_ppaxs[1][0].set_xlim(t.min(), t.max())
cc_ppaxs[1][0].set_yticks(np.arange(v_range[0], v_range[1] + 1, 50))
cc_ppaxs[1][0].set_yticks(np.arange(v_range[0], v_range[1] + 1, 25), minor = True)
...
# phase plane
lc = get_colorcode(v, dvdt, data_fc, norm = norm, cmap=cmap)
...
save_figures(cc_phaseplane, f'{frequency}_APs_pp_concat_first_AP', directories.figure_dir, darkmode_bool)

# annotations

t_peak = AP_params.at[0, 't_peaks'] + PGF_parameters['t_pre']
v_thres = AP_params.at[0, 'v_threshold']
t_thres = AP_params.at[0, 't_threshold'] + PGF_parameters['t_pre']

v_ampl = AP_params.at[0, 'v_amplitude']
v_HM = AP_params.at[0, 'v_HM']
t1_HM = AP_params.at[0, 't1_HM'] + PGF_parameters['t_pre']
t2_HM = AP_params.at[0, 't2_HM'] + PGF_parameters['t_pre']
FWHM = AP_params.at[0, 'FWHM']

cc_ppaxs[1][0].scatter(t_thres, v_thres,
                       marker = 'x',
                       s = 50,
                       c = color_dict['primecolor'])

cc_ppaxs[1][0].vlines(t_peak, v_thres, v_thres+v_ampl,
                      colors = color_dict['primecolor'])

cc_ppaxs[1][0].hlines(v_HM, t1_HM, t2_HM,
                      colors = color_dict['primecolor'])

plt.show()


save_figures(cc_phaseplane, f'{frequency}_APs_pp_concat_first_AP_annotated', directories.figure_dir, darkmode_bool)
